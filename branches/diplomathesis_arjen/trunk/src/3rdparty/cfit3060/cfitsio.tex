...
to 2**31 bytes = 2.1 GB in size, but this barrier is gradually being broken.
...
\item
FITS tables containing more than 2.1 G rows (given by the NAXIS2 keyword),
...
'll' ('el' 'el') to distinquish them from the 32-bit integer
...
\item[2 ] Return the specified keyword.  In the first routine,
    ...
    trailing blank characters stripped off.
...
\item[4 ] Copy an n-dimensional image in a particular row and column of a
    ...
    Any WCS keywords associated with the input column image will be tranlated
...
NAXIS1 pixels, (except when using HCOMPRESS, where the default is
to compress the entire image as a single big tile).
For example, a 800 x 800 x 4 pixel data cube would be
...
Currently, 4 image compression algorithms are supported:  Rice, GZIP,
HCOMPRESS, and PLIO.  Rice and GZIP are general purpose algorithms that can be
used to compress almost any image.  HCOMPRESS is very effective for
astronomical images.  The PLIO  algorithm is
more specialized and was developed for use in the IRAF data analysis
system to store
...
process.  The HCOMPRESS algorithm can also be used in a 'lossy' mode to achieve
a greater degree of image compression (by specifying a 'scale' parameter
value greater than 1).   To set the scale parameter effectively, one must
know the RMS noise in the image pixel values;  The fits\_rms\_float
and fits\_rms\_short routines in CFITSIO are provided to estimate
the RMS value in an image.
...
than, for example, simply using GZIP to compress the whole FITS file
image, but it also means that
...
The HCOMPRESS algorithm has 2 parameters.  The first is the scale
factor that controls the amount of compression.  The default value
of scale = 0 (or 1) results in lossless compression.  Values of scale greater
than 1 will produce larger amounts of 'lossy' compression.  As a rule
of thumb setting scale to about 2 times the RMS noise in the
sky background of typical astronomical image will compress the image
by about a factor of 10, with minimal loss of significant information.
Larger values of scale will produce greater compression, but with more
loss of information. The fits\_rms\_float and fits\_rms\_short routines
can be used to calculate the RMS value in an image.

The second HCOMPRESS parameter controls whether to slightly smooth
the image after decompressing it, if it was 'lossy' compressed
(with smooth > 1).  Smoothing helps to remove some of the 'blockiness'
in the image, especially if it was highly compressed.  The default
is to not smooth the image (smooth = 0), but setting smooth to 1 (or
any non-zero value) will cause the image to be smoothed.

The 'noisebits' parameter is only relevant when compressing floating point
images (with BITPIX = 32 or 64).
The default value is 4.  Decreasing the value of noisebits
...
\begin{description}
\item[18]  Translate a keyword name into a new name, based on a set of patterns.
...
these cases, it is necessary to tranlate the names of the keywords associated
...
by 1.  Even more complex translations are sometines needed to convert the
...
\begin{description}
\item[19]  Translate the keywords in the input HDU into the keywords that are
...
\begin{description}
\item[20]  Parse the input string containing a list of rows or row ranges, and
...
\begin{description}
\item[21]  Check that the Header fill bytes (if any) are all blank.  These are the bytes
...
\begin{description}
\item[22]  Check that the Data fill bytes (if any) are all zero (for IMAGE or
...
\begin{description}
\item[23]  Estimate the root-mean-squared (RMS) noise in an image.
...
(http://fits.gsfc.nasa.gov/group.html). A group is a
...
in the input file name.  These routines are mainly for interal use
...
eqivalent to an appropriate 64-bit integer datatype on each platform.
