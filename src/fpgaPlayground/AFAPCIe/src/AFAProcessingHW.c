    ...
    uint32_t spectraIndex )
{
	...
	// create a preinitialized buffer
	for ( k = 0; k < AFA_COMPARE_BATCH_HW; k++ )
	{
		err[ k ] = FLT_MAX;
	}
    ...
    for ( j = 0; j < m_gridSizeSqr; j += AFA_COMPARE_BATCH_HW )
    {
        ...
        for ( i = 0; i < AFA_COMPARE_BATCH_HW; i++ )
        {
//#pragma HLS ARRAY_PARTITION variable=err complete dim=1
//#pragma HLS ARRAY_PARTITION variable=err block factor=10 dim=2
#pragma HLS dependence variable=err inter false

            float32_t error = 0.0f;

			// JSC-TODO: Optimize away (access to BRam)
