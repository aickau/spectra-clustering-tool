...
entity AFAProcess_HW_INTERFACE_AXILITE_SLAVE_s_axi is
generic (
...
end entity AFAProcess_HW_INTERFACE_AXILITE_SLAVE_s_axi;
...
architecture behave of AFAProcess_HW_INTERFACE_AXILITE_SLAVE_s_axi is
    ...
    constant ADDR_BASEADDR_DATA_0 : INTEGER := 16#18#;
    constant ADDR_BASEADDR_CTRL   : INTEGER := 16#1c#;
    ...
    signal int_baseAddr        : UNSIGNED(31 downto 0);
    ...
    begin
        if (ACLK'event and ACLK = '1') and ACLK_EN = '1' then
            if (ar_hs = '1') then
                ...
                when ADDR_BASEADDR_DATA_0 =>
                    rdata_data <= RESIZE(int_baseAddr(31 downto 0), 32);
                when others =>
                    rdata_data <= (others => '0');
                end case;
            end if;
...
-- ----------------------- Register logic ----------------
    ...
    baseAddr             <= STD_LOGIC_VECTOR(int_baseAddr);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_BASEADDR_DATA_0) then
                    int_baseAddr(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_baseAddr(31 downto 0));
                end if;
            end if;
        end if;
    end process;
